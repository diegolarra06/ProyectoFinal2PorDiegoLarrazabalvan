<!--
  ===========================================================
  TARJETA DE ANIMAL (componente reutilizable)
  Tema 9 PDF - Props (defineProps)
  Tema 9 PDF - Emits (defineEmits)
  Se usa en el catálogo y en favoritos.
  ===========================================================
-->
<script setup>
...
// URL de la imagen (la backend la devuelve como nombre, hay que componerla)
const imagenUrl = computed(() => {
  if (props.animal.imagenPrincipal) {
    return `http://localhost:8080/uploads/${props.animal.imagenPrincipal}`
  }
  return '/img/perrera.jpg' // imagen por defecto
})
  ...
  <article class="tarjeta">
    ...
    <div class="tarjeta-body">
      ...
      <div class="acciones">
        ...
        <button v-if="mostrarBotonFavorito"
                class="btn-fav"
...
.tarjeta-img { position: relative; height: 200px; overflow: hidden; }
...
.badge-disponible { background-color: var(--color-success); }
.badge-reservado  { background-color: var(--color-warning); }
.badge-adoptado   { background-color: var(--color-secundario); }
.tarjeta-body { padding: 15px; flex: 1; display: flex; flex-direction: column; }
.tarjeta-body h3 { margin-bottom: 6px; color: var(--color-primario); }
.info { font-size: 0.9rem; color: #666; margin-bottom: 8px; }
.personalidad { font-size: 0.9rem; flex: 1; }
.acciones { display: flex; gap: 8px; margin-top: 10px; }
.btn-fav {
  background: white; border: 1px solid var(--color-gris-medio);
  border-radius: var(--radius); padding: 8px 12px; cursor: pointer;
  color: var(--color-error); transition: var(--transition);
}
.btn-fav.activo { background: var(--color-error); color: white; }
</style>