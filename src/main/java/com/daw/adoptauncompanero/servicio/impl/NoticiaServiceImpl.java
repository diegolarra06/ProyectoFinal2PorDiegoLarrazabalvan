...
public class NoticiaServiceImpl implements NoticiaService {
    ...
    @Override
    @Transactional
    public Integer crearNoticia(String titulo, String subtitulo, String contenido,
    ...
    @Override
    @Transactional
    public Integer actualizarNoticia(Integer idNoticia, String titulo, String subtitulo,
                                     String contenido, String autor, Boolean publicada) {
        NoticiaEntity n = noticiaRepository.findById(idNoticia).orElse(null);
        if (n == null) return 0;
        if (titulo != null && !titulo.isBlank()) n.setTitulo(titulo);
        n.setSubtitulo(subtitulo);
        if (contenido != null && !contenido.isBlank()) n.setContenido(contenido);
        n.setAutor(autor);
        if (publicada != null) n.setPublicada(publicada);
        n.setFechaModificacion(LocalDateTime.now());
    ...
    public Integer borrarNoticia(Integer idNoticia) {
        if (!noticiaRepository.existsById(idNoticia)) return 0;
